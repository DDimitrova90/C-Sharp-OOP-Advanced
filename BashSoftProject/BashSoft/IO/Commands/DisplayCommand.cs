namespace BashSoft.IO.Commands
{
    ...
    public class DisplayCommand : Command, IExecutable
    {
        ...
        public override void Execute()
        {
            ...
            {
                Comparer<IStudent> studentComparator = this.CreateStudentComparator(sortType);
                ISimpleOrderedBag<IStudent> list = this.repository.GetAllStudentsSorted(studentComparator);
                OutputWriter.WriteMessageOnNewLine(list.JoinWith(Environment.NewLine));
            }
            else if (entityToDisplay.Equals("courses", StringComparison.OrdinalIgnoreCase))
            {
                Comparer<ICourse> courseComparator = this.CreateCourseComparator(sortType);
        ...
        private Comparer<ICourse> CreateCourseComparator(string sortType)
        {
        ...
        private Comparer<IStudent> CreateStudentComparator(string sortType)
        {
            if (sortType.Equals("ascending", StringComparison.OrdinalIgnoreCase))
            {
